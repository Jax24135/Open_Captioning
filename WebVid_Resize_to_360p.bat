...
:: Resizes and creates (2) 360p video files
@echo off
...
::"C:\ffmpeg\bin\ffmpeg" -i "%videoFile%" -vf scale="640:360" -crf 18 -threads 3 "%videoFileNoExt%.tmp.mp4"
...
"C:\ffmpeg\bin\ffmpeg" -i "%videoFile%" -b:v 1.5M -c:a copy -vf scale="640x360"-threads 3 -loglevel info "%videoFileNoExt%_REV_HR.mp4"
"C:\ffmpeg\bin\ffmpeg" -i "%videoFile%" -b:v 0.45M -c:a copy -vf scale="640x360" -threads 3 -loglevel info "%videoFileNoExt%_REV_LR.mp4"
